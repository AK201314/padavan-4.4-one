...
static const struct {
    ...
    "adbybycompatible", oAdbybyCompatible}, {
...
config_read(const char *filename)
{
    ...
    debug(LOG_INFO, "Starting WiFidog , Reading configuration file '%s'", filename);
        ...
        if (p1 && p1[0] != '\0') {
            ...
            if ((strncmp(s, "#", 1)) != 0) {
                ...
                case oProxyPort:
                    ...
                    break;
                case oAdbybyCompatible:
                    sscanf(p1, "%d", &config.adbyby_compatible);
                    break;
                case oSSLCertPath:
