...
iptables_fw_init(void)
{
    ...
    if ((adbyby_compatible = config_get_config()->adbyby_compatible) != 0) {
        if ((adbyby_compatible = config_get_config()->adbyby_compatible) != 1) 
            iptables_do_command("-t nat -N ADBYBY");
        iptables_do_command("-t nat -A " CHAIN_KNOWN
                        " -p tcp -m multiport --dport 80,8080,3128,8081,9080 -j ADBYBY");
        iptables_do_command("-t nat -A " CHAIN_VALIDATE
                        " -p tcp -m multiport --dport 80,8080,3128,8081,9080 -j ADBYBY");
    }
    ...
    iptables_do_command("-t nat -A " CHAIN_TO_INTERNET " -j " CHAIN_AUTHSERVERS);
    iptables_do_command("-t nat -A " CHAIN_TO_INTERNET " -m mark --mark 0x%u -j " CHAIN_KNOWN, FW_MARK_KNOWN);
    iptables_do_command("-t nat -A " CHAIN_TO_INTERNET " -m mark --mark 0x%u -j " CHAIN_VALIDATE, FW_MARK_PROBATION);
    iptables_do_command("-t nat -A " CHAIN_TO_INTERNET " -j " CHAIN_UNKNOWN);

    iptables_do_command("-t nat -A " CHAIN_UNKNOWN " -j " CHAIN_GLOBAL);
    iptables_do_command("-t nat -A " CHAIN_KNOWN " -j ACCEPT");
    iptables_do_command("-t nat -A " CHAIN_VALIDATE " -j ACCEPT");
    if (got_authdown_ruleset) {
...
iptables_fw_counters_update(void)
{
    ...
    while (output && !(feof(output))) {
        rc = fscanf(output, "%*s %llu %*s %*s %*s %*s %*s %15[0-9.] %*s %*s %*s %*s 0x%*u", &counter, ip);
        if (2 == rc && EOF != rc) {
            /* Sanity */
            ...
            if ((p1 = client_list_find_by_ip(ip))) {
                ...
                debug(LOG_ERR,
                      ...
                      ip);
                debug(LOG_ERR, "Preventively deleting firewall rules for %s in table " CHAIN_OUTGOING, ip);
                iptables_fw_destroy_mention("mangle", CHAIN_OUTGOING, ip);
                debug(LOG_ERR, "Preventively deleting firewall rules for %s in table " CHAIN_INCOMING, ip);
